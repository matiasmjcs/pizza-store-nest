...
export class ShoppingCartsService {
  ...
  async addProduct(id: number, updateShoppingCartDto: UpdateShoppingCartDto) {
    ...
    if(!product) {throw new BadRequestException("no se a encontrado el producto")}
    const shoppingcart = await this.shoppingCartRepository.findOneBy({id})
    if(!shoppingcart) {throw new BadRequestException("no se a encontrado el carro de compras")}
    return await this.shoppingCartRepository.save({id:shoppingcart.id,products: [...shoppingcart.products,product]});
  }
